...
> 初稿：react 第一步都思考然后行动观察不行再来一轮，plan_and_solve 开始的计划就想好了蓝图接着再行动，reflection 根据条件来筛选审查有输出格式

- ReAct ✅ 每轮 `Thought→Action→Observation` 循环，不满意进下一轮到 Finish。"再来一轮"意思对。
- Plan-and-Solve ✅ 先一次性生成完整计划，再顺序执行、中途不回头不改。
- Reflection ⚠️ 不是"筛选工具"。本质是**生成初稿→评审→优化**打磨同一产出；"输出格式"是任务约束非范式特征。

### Q2　智能家居控制助手选哪种范式？
> 初稿：reflaction 可以打磨调节，多个设备有不同的需求 reflaction 能够高质量地满足

- ❌ Reflection 只打磨一段输出（如语音播报），**协调不了多个设备**（灯/空调/窗帘分别调用）。
- ✅ 应选 **ReAct 为主**（每步决定调哪个设备、传什么参数）+ Reflection 润色话术 + Plan-and-Solve 管固定场景（晚安模式）。单一 Reflection 不行。

### Q3　三种范式能否组合？
> 初稿：react and reflaction 迭代化调用工具，温度自动调节 / 情绪音乐 / 复杂数学解题

- 方向对。标准混合：**Plan（高层规划）→ ReAct（每步执行+工具）→ Reflection（关键输出质检）**。
- 纠正举例：温度自动调节偏 **ReAct**（感知→决策→执行）；情绪音乐可加 Reflection 打磨理由；数学解题用 Plan-and-Solve 拆步 + Reflection 验算。
...
- 动态重规划：每步执行后验证，失败则从**当前状态重生成剩余计划**（不从头）。
